...
import { productsPage, productDetailsPage } from "../support/pages/index";
  ...
  it('Validate product detail link', () => {
    productsPage.productList.each(product => {
      ...
      cy.visit(productDetailsLink).then(() => {
        ...
        productDetailsPage.productTitle.invoke('text').then(text => { // forma de captura o elemento do titulo do produto 
          expect(text).to.be.equal(productTitle)
        })
        productDetailsPage.variations.find('[for]').should('have.length', 2).and('be.visible')
      })
    })
  });
});